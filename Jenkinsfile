pipeline {
    ...
    stages {
        stage ('Check Commit Message') {
            steps {
                script {
                    ...
                    echo "Paperline will continue. No [skip ci] tag found in commit message."
                }
            }
        }
        ...
        stage ('Checkout Code') {
            steps {
                script {
                    def repoUrl = 'https://github.com/bwa-project-example/user-service.git'
        ...
        stage ('Commit and Push Changes') {
            steps {
                script {
                    ...
                    git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB__CREDENTIALS_PSW}@git@github.com:Mini-Soccer-Project/user-service.git
    ...
    stage('Deploy to Remote Server') {
        steps {
            script {
                def targetDir = "/home/faisalilham/mini-soccer-project/user-service"
                ...
                if [ -d "${targetDir}/.git" ]; then
                    ...
                    git clone -b "${TARGET_BRANCH}" git@github.com:bwa-project-example/user-service.git "${targetDir}"
                    cd "${targetDir}"
                fi
